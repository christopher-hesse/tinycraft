...
def main() -> None:
    ...
    check_python_exes_runnable(
        python27_exe_arg=args.python27_exe, python36_exe_arg=args.python36_exe
    )
    files_to_test = determine_files_to_test(
        typeshed_location=typeshed_location, subdir_paths=subdir_paths
    )
    run_all_tests(
...
class PathMatcher:
    def __init__(self, patterns: Sequence[str]) -> None:
        self.matcher = (
            re.compile(r"({})$".format("|".join(patterns))) if patterns else None
        )
...
def run_pytype(
    *, filename: str, python_version: str, python_exe: str, typeshed_location: str
) -> Optional[str]:
...
def _get_module_name(filename: str) -> str:
    ...
    return (
        ".".join(_get_relative(filename).split(os.path.sep)[2:])
        .replace(".pyi", "")
        .replace(".__init__", "")
    )
...
def can_run(exe: str, *, args: List[str]) -> bool:
    try:
        subprocess.run(
            [exe] + args, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        )
    except OSError:
        return False
    else:
...
def _is_version(path: str, version: str) -> bool:
    return any(
        "{}{}{}".format(d, os.path.sep, version) in path for d in TYPESHED_SUBDIRS
    )
...
def check_subdirs_discoverable(subdir_paths: List[str]) -> None:
    for p in subdir_paths:
        if not os.path.isdir(p):
            raise SystemExit(
                "Cannot find typeshed subdir at {} (specify parent dir via --typeshed-location)".format(
                    p
                )
            )
...
def determine_files_to_test(
    *, typeshed_location: str, subdir_paths: Sequence[str]
) -> List[Tuple[str, int]]:
    ...
    for root, _, filenames in itertools.chain.from_iterable(
        os.walk(p) for p in subdir_paths
    ):
