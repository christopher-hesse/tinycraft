...
parser = argparse.ArgumentParser(
    description="Test runner for typeshed. "
    "Patterns are unanchored regexps on the full path."
)
parser.add_argument("-v", "--verbose", action="count", default=0, help="More output")
parser.add_argument(
    "-n", "--dry-run", action="store_true", help="Don't actually run mypy"
)
...
parser.add_argument(
    "--platform", help="Run mypy for a certain OS platform (defaults to sys.platform)"
)
parser.add_argument(
    "--warn-unused-ignores",
    action="store_true",
...
def main():
    ...
    with open(os.path.join(os.path.dirname(__file__), "mypy_blacklist.txt")) as f:
        blacklist = re.compile(
            "(%s)$"
            % "|".join(re.findall(r"^\s*([^\s#]+)\s*(?:#.*)?$", f.read(), flags=re.M))
        )
        ...
        for root in roots:
            ...
            for name in names:
                ...
                if ext in [".pyi", ".py"]:
                    ...
                    os.path.join(full, "__init__.pyi")
