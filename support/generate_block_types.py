...
def main():
    ...
    while offset < len(lines):
        block_id = lines[offset]
        # block_title = lines[offset+1]
        block_name = lines[offset + 2][1:-1].split(":")[1]
        offset += 3
        if ":" in block_id:
            continue
        enum_identifier = "BlockType_" + block_name.replace("_", " ").title().replace(
            " ", ""
        )
        # print(f"{enum_identifier} = {block_id},")
        block_types[int(block_id)] = enum_identifier

    for i in range(256):
        value = block_types.get(i, "<unused>")
        print(f"// {value} = {i}")
        print(
